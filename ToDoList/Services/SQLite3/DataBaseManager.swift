...
protocol DataBaseManagerProtocol {
    func save(todoItems: [TodoItem]) throws
    func loadDb() throws -> [TodoItem]
...
extension DataBaseManager: DataBaseManagerProtocol {
    func save(todoItems: [TodoItem]) throws {
        for todoItem in todoItems {
            let saveStatementString = todoItem.sqlReplaceStatement
    ...
    func loadDb() throws -> [TodoItem] {
        ...
        if sqlite3_prepare_v2(dataBase, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
            ...
            throw DataBaseManagerError.errorLoadFromDB
        }
    ...
    func update(todoItem: TodoItem) throws {
        ...
        if sqlite3_prepare_v2(dataBase, updateStatementString, -1, &updateStatement, nil) == SQLITE_OK {
            let text = todoItem.text as NSString
            let importance = todoItem.importance.rawValue as NSString
            let isDone = todoItem.isDone ? 1 : 0
            let createdAt = String(todoItem.createdAt.dateIntValue ?? 0)
            let hexColor = todoItem.hexColor as NSString
            let lastUpdatedBy = todoItem.lastUpdatedBy as NSString
            let id = todoItem.id as NSString
            
            if let deadline = todoItem.deadline, let deadlineIntValue = deadline.dateIntValue {
                sqlite3_bind_int64(updateStatement, 3, Int64(deadlineIntValue))
            } else {
                sqlite3_bind_null(updateStatement, 3)
            }
            
            if let changesAt = todoItem.changesAt, let changesAtIntValue = changesAt.dateIntValue {
